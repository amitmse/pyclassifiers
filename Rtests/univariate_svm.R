...
## score
model <- svm(Species ~ Petal.Width, data=d, kernel='linear', cross=nrow(d))
...
# estimate cutpoint, using our knowledge that versicolors have shorter
# petal width...
units = .1
for (i in seq(from=min(d$Petal.Width), to=max(d$Petal.Width), by=units)) {
    if (predict(model, data.frame(Petal.Width=i)) != hit)
        break
}
cat('Cutpoint:', i - units, '\n')
